...
/**
 * Fungsi untuk parsing teks dengan tanda ^
 * Contoh: "Belajar ^Java^ dan ^PHP^" 
 * akan menjadi React nodes dengan <span>
 */
    ...
    >
      ...
      >
        ...
        <motion.div className="text-center space-y-1" variants={fadeIn(0.4)}>
          ...
          <p className="text-sm font-bold text-satin-sheen-gold-900">{profileData.title}</p>
        </motion.div>
        ...
        >
          {profileData.bio.map((paragraph, i) => (
            <p key={i} className="indent-8">
              {parseStyledText(paragraph, "font-bold text-satin-sheen-gold-700")}
            </p>
          ))}
        </motion.div>
      </motion.div>
